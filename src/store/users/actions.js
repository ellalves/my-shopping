...
const create = ({ commit }, params) => {
  ...
  data['array'] = [params.name, params.image, params.mark_id]
  return new Promise((resolve, reject) => {
    let db = Vue.prototype.$sqlite
    db.transaction(function (tx) {
      tx.executeSql(data['sql'], data['array'], function (tx, results) { // Successo
...
const update = ({ commit }, params) => {
  ...
  let data = []
  data['sql'] = `UPDATE products SET name = ?, price = ?, image = ? WHERE id = ?`
  data['array'] = [params.name, params.price, params.image, params.id]
  let db = Vue.prototype.$sqlite
  return new Promise((resolve, reject) => {
    db.transaction(function (tx) {
      tx.executeSql(data['sql'], data['array'], function (tx, results) { // Successo
